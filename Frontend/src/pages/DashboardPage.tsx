...
const mockNews = [
  {
    ...
    summary: 'A사의 새로운 AI 모델이 복잡한 다단계 추론 테스트에서 인간과 대등한 성능을 기록하며 기술계에 큰 충격을 주었습니다. 이는 AI가 단순 정보 검색을 넘어 창의적 문제 해결의 영역으로 진입하고 있음을 시사합니다.',
  ...
  },
  {
    ...
    summary: '미국 연방준비제도 의장이 인플레이션 둔화 신호를 긍정적으로 평가하며 당분간 금리를 동결할 것임을 시사했습니다. 이 발언으로 뉴욕 증시는 일제히 상승 마감했습니다.',
    source: '머니투데이',
    timestamp: '4시간 전',
  },
  {
    ...
    summary: '정부가 심각한 저출산 문제 해결을 위해 신혼부부 및 출산 가구에 대한 주택 공급 확대와 대출 지원을 포함한 파격적인 주거 안정 대책을 발표했습니다.',
    source: '연합뉴스',
    timestamp: '1시간 전',
  },
  {
    ...
    summary: '12.12 군사반란을 다룬 영화 \'서울의 봄\'이 개봉 한 달 만에 천만 관객을 돌파하며 올해 최고 흥행작으로 등극했습니다. 작품성과 대중성을 모두 잡았다는 평가입니다.',
    source: '씨네21',
    timestamp: '8시간 전',
  },
];
...
const NewsCard = ({ newsItem }: { newsItem: typeof mockNews[0] }) => (
  <div className="card flex flex-col p-6 hover:shadow-lg transition-shadow duration-300">
    <div className="flex-grow">
      ...
      <h3 className="text-lg font-bold text-gray-900 mb-2 leading-snug">{newsItem.title}</h3>
      <p className="text-gray-600 text-sm leading-relaxed">{newsItem.summary}</p>
    </div>
  ...
  if (loading) {
    return (
      <div className="w-full h-96 flex items-center justify-center">
        <div className="animate-spin rounded-full h-24 w-24 border-b-2 border-brand-primary"></div>
      </div>
    );
  }
