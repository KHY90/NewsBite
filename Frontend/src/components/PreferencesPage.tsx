...
const PreferencesPage: React.FC = () => {
  ...
  const handleToggle = (id: number, type: 'category' | 'company') => {
    const setter = type === 'category' ? setSelectedCategories : setSelectedCompanies;
    setter(prev => 
      prev.includes(id) 
        ? prev.filter(i => i !== id)
        : [...prev, id]
  ...
  const handleSavePreferences = async () => {
    ...
    try {
      ...
      console.log('Saving:', { categories: selectedCategories, companies: selectedCompanies });
    ...
    <div className="container py-12">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-12">
          {/* 카테고리 섹션 */}
          <div>
            <h2 className="text-h3 font-semibold text-gray-900 mb-4">뉴스 카테고리</h2>
          ...
          {/* 기업 섹션 */}
          <div>
            <h2 className="text-h3 font-semibold text-gray-900 mb-4">관심 기업</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
...
const PreferenceCard = ({ item, isSelected, onToggle }: any) => (
  <div
    className={`card p-4 flex items-center justify-between cursor-pointer transition-all duration-200 ${isSelected ? 'border-brand-primary ring-2 ring-brand-primarySoft' : 'border-line-soft hover:border-line-strong'}`}
    onClick={onToggle}
  >
    ...
    <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 ${isSelected ? 'border-brand-primary bg-brand-primary' : 'border-gray-300'}`}>
      {isSelected && (
