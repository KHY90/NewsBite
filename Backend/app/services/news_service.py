# 이 파일은 뉴스 데이터와 직접적으로 상호작용하는 비즈니스 로직을 담당하는 서비스 계층입니다.
# 데이터베이스와 통신하여 뉴스를 생성, 조회, 검색, 업데이트하는 핵심 기능들을 제공합니다.
# API 엔드포인트는 이 서비스의 메소드를 호출하여 필요한 데이터를 얻거나 변경합니다.
#
# - `NewsService` 클래스: 뉴스 관련 데이터베이스 작업을 캡슐화합니다.
#   - `create_news_article`: 크롤링된 뉴스 아이템을 데이터베이스에 저장합니다. 중복을 체크하고,
#                            필요 시 새로운 카테고리를 생성합니다.
#   - `get_recent_news`, `get_trending_news`, `search_news`: 다양한 조건(최신순, 인기순, 키워드)에
#                                                          따라 뉴스 목록을 조회합니다.
#   - `get_personalized_news`: (구현 예정) 사용자 맞춤형 뉴스를 제공하는 로직이 위치할 곳입니다.
#   - `increment_view_count`: 뉴스의 조회수를 1 증가시킵니다.
#   - `get_news_statistics`: 전체 뉴스 수, 카테고리별/소스별 통계 등 유용한 정보를 제공합니다.
