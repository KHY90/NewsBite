# 이 파일은 뉴스 데이터와 관련된 다양한 API 엔드포인트를 정의합니다.
# 최신 뉴스, 트렌딩 뉴스, 카테고리별 뉴스 조회, 뉴스 검색, 상세 조회,
# 통계 조회 등 뉴스 관련 대부분의 클라이언트 요청을 처리합니다.
# 또한, 관리자 기능으로 뉴스 크롤링을 수동으로 실행하거나 스케줄러 상태를
# 확인할 수 있는 엔드포인트도 포함합니다.
